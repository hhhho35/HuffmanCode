...
#include"stdafx.h"
#include "string.h"								// 串类
#include "HuffmanBTreeNode.h"					// 哈夫曼树结点类模板
...
// 操作结果：由字符,权值和字符个数构造哈夫曼树
{
	...
	for (pos = 1; pos <= n; pos++)
	{	// 求n个叶结点字符的编码
		list<CharType> charCode;			// 暂存叶结点字符编码信息
		list<CharType>::iterator it;
		...
		{	// 从叶结点到根结点逆向求编码
			...
			if (nodes[parent].leftChild == child)
				charCode.insert(it, '0');// 左分支编码为'0'
			else 
				charCode.insert(it, '1');	// 右分支编码为'1'
		}
		// charCode中存储字符编码
		int j = charCode.size();
		int i = 0;
		...
		{
			//LeafCharCodes[pos].append(*it);
			a[i] = *it;
			i++;
		}
		a[i] = '\0';
		string str= a;
		LeafCharCodes[pos]=str;
	}
...
// 操作结果：对编码串strCode进行译码,返回编码前的字符序列
{
	...
	{	// 处理每位编码
		...
		if (nodes[curPos].leftChild == 0 && nodes[curPos].rightChild == 0)
		{
			...
			charList.insert(it , LeafChars[curPos]);
